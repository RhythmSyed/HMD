...
void epaper_draw_activity_mode(epaper_handle_t device, int * position){
...
void epaper_draw_pairing_mode(epaper_handle_t device){
    char steps[sizeof(display_data.step_count)];

    iot_epaper_draw_string(device, 10, 10, "PAIRING", &epaper_font_24, COLORED);
    iot_epaper_draw_string(device, 10, 33, "MODE", &epaper_font_24, COLORED);
    iot_epaper_draw_rectangle(device, 5, 5, 195, 65, COLORED);

    // sprintf(steps, "x=%+7.3f y=%+7.3f z=%+7.3f",
    //     display_data.imu_data.ax, display_data.imu_data.ay, display_data.imu_data.az);
    
    sprintf(steps, "PLEASE PAIR HMD");
    iot_epaper_draw_string(device, 10, 90, steps, &epaper_font_16, COLORED);
...
void epaper_draw_pairingsuccess_mode(epaper_handle_t device){
    char steps[sizeof(display_data.step_count)];
    ...
    sprintf(steps, "PAIRED!");
    iot_epaper_draw_string(device, 10, 90, steps, &epaper_font_16, COLORED);
    iot_epaper_draw_string(device, 10, 100, "Choose mode on App", &epaper_font_12, COLORED);
...
{  
    ...
    while(1){
        ...
        else if (display_data.current_mode == PAIRING_SUCCESS) {
            epaper_draw_pairingsuccess_mode(device);
        }
